      ...
      return (
        <>
          <button
            ...
            className="w-full flex items-center gap-2 py-2 px-3 text-sm text-gray-700 font-medium rounded-lg hover:bg-gray-100"
          >
          ...
          <div className="fixed top-16 left-0 w-full bg-white shadow-lg rounded-b-xl flex flex-col items-center py-6 gap-4 md:hidden z-50 transition-transform duration-300">
            {categories.map((cat) => (
              <Link
                key={cat.name}
                href={cat.link}
                className="w-full text-center py-2 text-[#008080] font-medium hover:text-green-600 hover:bg-gray-100 rounded-md"
                onClick={() => setMenuOpen(false)}
              >
                {cat.name}
              </Link>
